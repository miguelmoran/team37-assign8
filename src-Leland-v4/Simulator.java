...
public class Simulator {
	...
	boolean[] selectedInstructors = new boolean[MAXSize];
		...
		if ( numInstructorsSelected < MAXInstructors) {
			...
			if (selection < assignmentList.size() && selection >=0 ) {
				if (unSelectedAssignments[selection]){		// this is a valid row or index
						...
						if (selection <assignmentList.size())
							a=assignmentList.get(selection);
					
						if (!selectedInstructors[a.getInstructor().getId()]) {
									selectedInstructors[a.getInstructor().getId()] = true;
									//DEBUG
									System.out.println("adding row "+selection+" instructor "+a.getInstructor().getId());
		...
		if (numInstructorsSelected > 0){
			...
			if (selection < assignmentList.size() && selection >=0 ) {
				if (selectedAssignments[selection]){		// this row was previously added
					...
					selectedInstructors[a.getInstructor().getId()] = false;
	...
	protected void initializeInstructors () {
